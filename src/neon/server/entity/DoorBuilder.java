...
public class DoorBuilder implements EntityBuilder<Door> {
	...
	@Override
	public Entity build(long uid, Door resource) {
		Entity door = builder.build(uid, resource);
		door.setComponent(new DoorInfo(uid));
		door.setComponent(new Graphics(uid, resource.closed, resource.color));
		return door;
	}
}
